<template>
    <ion-button fill="clear" expand="block" @click="setOpenRequests(true)">Demandes de suppression</ion-button>
    <ion-modal :is-open="isOpenRequests" @didDismiss="setOpenRequests(false)">
        <ion-header>
            <ion-toolbar>
                <ion-title class="ion-text-center">Requêtes de suppression</ion-title>
                                ...
                                <ion-row class="ion-justify-content-center">
                                    <ion-item color="ligth">
                                        <ion-select interface="popover" placeholder="Réponse"
                                            ...
                                            <ion-select-option value="yes">Accepter la suppression</ion-select-option>
                                            <ion-select-option value="no">Refuser la suppression</ion-select-option>
                                        </ion-select>
                                    </ion-item>
                                </ion-row>
                            </ion-col>
...
import { COLLECTION_REQUESTS, getRequests, getDocumentByDocId, getImageUrl, setRequest } from '@/services/firebase-service';
...
async function showList(docData: DocumentData) {
    ...
    if (requestData != null) {
        if (docData.typeRequete == 'Supprimer' && docData.statutRequete == 'En attente') {
...
async function getReqResponse(ev: any, collection: string, itemId: number) {
    ...
    if (response == 'yes') {
        setRequest(collection, itemId, 'Accepté', 'Supprimer')

        const toast = await toastController.create({
            message: "Demande de suppression acceptée",
            duration: 3000
        })
        await toast.present()
    } else {
        setRequest(collection, itemId, 'Refusé', 'Supprimer')
